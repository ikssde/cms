...
class Opt_Function
{
	...
	static public function pluralize($number, $singularForm)
	{
		...
		if(sizeof($args) == 0)
		{
			...
			{
				throw new Opt_Runtime_Exception('Error in pluralize() function: the translation interface does not support pluralization.');
			}
		...
		else
		{
			...
			{
				throw new Opt_Runtime_Exception('Error in pluralize() function: unknown grammar form number: '.$argument.'.');
			}
	...
	static public function autoLink($text, $class = null, $target = '_blank')
	{
		...
		return preg_replace_callback('/(((http|https|ftp|ftps|gopher)\:\/\/)|(www\.)|(ftp\.))([a-zA-Z0-9\_\-]+\@)?(([0-9a-fA-F\:]{6,39})|(([a-zA-Z0-9\-\_]+\.)*[a-zA-Z0-9\-\_]+(\:[0-9]{1,5})?)|(\[[0-9a-fA-F\:]{6,39}\]\:[0-9]{1,5}))(\/[a-zA-Z0-9\_\-\&\;\?\/\.\=\+\#]*)?/is',
			function ($matches) use ($extra)
			{
				...
				elseif(strpos($addr, 'ftp.') === 0)
				{
					$addr == 'ftp://'.$addr;
				}
				return '<a href="'.$addr.'"'.$extra.'>'.$matches[0].'</a>';
			},
